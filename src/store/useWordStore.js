        ...
        (set, get) => ({
            ...
            importSeedWords: (entries = []) => {
                ...
                entries.forEach((entry) => {
                    ...
                    seen.add(clean);
                    incomingWords.push(createWord(clean, {
                        tag: 'seed',
            ...
            removeUnstudiedSeedWords: () => {
                const removableIds = new Set(
                    get().words
                        .filter((word) => word.isSeeded && !hasMeaningfulProgress(word))
                        .map((word) => word.id)
                );
            ...
            promoteFromSRS: (wordId, srsResult) => {
                ...
                const recentLapses = srsResult?.lastReviewResult === 'nao_lembro' ? (word.recentLapses ?? 0) + 1 : 0;
