...
function legacyActionCount(day = {}) {
    return (day.readerWords || 0) + (day.builderExercises || 0) + (day.flashcardReviews || 0) + (day.productionWrites || 0);
}
...
function getLatestActiveDay(dailyStats = {}) {
    return Object.keys(dailyStats)
        .filter((dayKey) => legacyActionCount(dailyStats[dayKey]) > 0 || getStudySecondsForDay(dailyStats[dayKey]) > 0)
        .sort()
        .at(-1) ?? null;
}
...
function buildAchievements(state) {
    ...
    if (getStudyDaysInLast(7, state.dailyStats, DEFAULT_MIN_SESSION_MINUTES) >= 4) achievements.push('weekly_rhythm');
...
function addStudyTime(day, timestamp = Date.now()) {
    ...
    const boundedSeconds = Math.min(180, Math.max(30, Number.isFinite(diffSeconds) ? diffSeconds : 60));
...
function maybeResolveAutoAdjust(state, { currentLevel, autoAdjustEnabled }) {
    ...
    const accuracy = windowSize > 0
        ? recentResults.filter(Boolean).length / windowSize
        : 0;
    ...
    if ((exerciseCount - (state.autoAdjustMeta?.lastAdjustedOnExerciseCount || 0)) < AUTO_ADJUST_WINDOW) {
        return {
            state: {
                ...state,
                autoAdjustMeta: baseMeta,
...
export const useProgressStore = create(
    persist(
        (set) => ({
            ...
            recordBuilderExercise: ({ wordId, firstTry, recycled, success = true, currentLevel, autoAdjustEnabled = false, mode = 'assembly' }) => {
                const dayKey = getDayKey();
                let adjustment = null;
                ...
                set((state) => {
                    let next = withDayRecord(state, dayKey);
                    const journey = next.wordJourney[wordId] || {};
                    const nextRecentResults = [...(next.builderRecentResults || []), success].slice(-AUTO_ADJUST_WINDOW);
                    ...
                    next = {
                        ...
                        dailyStats: {
                            ...
                            [dayKey]: {
                                ...
                                transformExercises: next.dailyStats[dayKey].transformExercises + (mode === 'transform' ? 1 : 0),
...
export { calculateRetentionRate, calculateStreakStats, getDayKey, getStudySecondsForDay, LEVEL_ORDER };
