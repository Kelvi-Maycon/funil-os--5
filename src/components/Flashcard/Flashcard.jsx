    ...
    <div className="w-full flex flex-col gap-6">
      ...
      {!flipped ? (
        <div className="flex justify-center mt-4">
          <button className="bg-neutral-900 text-white hover:bg-black px-10 py-5 rounded-3xl font-black shadow-lg transition-transform hover:-translate-y-1 flex items-center gap-3 text-lg w-full md:w-auto" onClick={() => setFlipped(true)}>
        ...
        <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 shadow-soft border border-neutral-100 animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-6 text-center">Como você se saiu?</div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ['nao_lembro', 'Não lembro', 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100 hover:border-red-300'],
              ['dificil', 'Difícil', 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100 hover:border-amber-300'],
              ['bom', 'Bom', 'bg-violet-50 text-violet-700 border-violet-200 hover:bg-violet-100 hover:border-violet-300'],
              ...
              <button key={rating} className={`flex flex-col items-center justify-center p-4 rounded-3xl border transition-all hover:-translate-y-1 hover:shadow-md ${colorClass}`} onClick={() => onRate(rating)}>
                ...
                <span className="text-[11px] font-bold opacity-80 bg-white/60 px-2 py-0.5 rounded-md border border-black/5">+{formatInterval(previewInterval(card, rating))}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mt-4">
        <div className="px-3 py-1.5 bg-neutral-100 text-neutral-600 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-neutral-200/60">Intervalo: {formatInterval(card.interval)}</div>
        <div className="px-3 py-1.5 bg-neutral-100 text-neutral-600 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-neutral-200/60">EF: {(card.easeFactor || 2.5).toFixed(2)}</div>
        <div className="px-3 py-1.5 bg-neutral-100 text-neutral-600 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-neutral-200/60">Revisões: {card.reviewCount || 0}</div>
      </div>
    </div>
  );
}
            ...
            <main className="w-full mt-2 lg:mt-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col justify-between relative overflow-hidden group">
                        ...
                        <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-4">Pendentes hoje</div>
                        <div className={`text-5xl font-black ${dueCount > 0 ? 'text-violet-600' : 'text-neutral-300'}`}>{dueCount}</div>
                    </div>
                    <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col justify-between relative overflow-hidden">
                        <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-4">Total de cards</div>
                        <div className="text-5xl font-black text-neutral-900">{totalCards}</div>
                    </div>
                    <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col justify-between relative overflow-hidden">
                        <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-4">Retenção 7d</div>
                ...
                {totalCards === 0 ? (
                    <div className="bg-white rounded-[2rem] p-12 shadow-soft border border-neutral-100 flex flex-col items-center justify-center text-center">
                        ...
                        <div className="flex gap-4 flex-wrap mt-8">
                            <button className="bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white px-8 py-3.5 rounded-full font-bold shadow-lg transition-transform hover:-translate-y-0.5" onClick={() => navigate('/practice')}>Ir para prática</button>
                            <button className="bg-white text-neutral-700 border border-neutral-200 hover:bg-neutral-50 px-8 py-3.5 rounded-full font-bold shadow-sm transition-transform hover:-translate-y-0.5" onClick={() => navigate('/reader')}>Voltar ao reader</button>
                        </div>
                    </div>
                ) : dueCount === 0 ? (
                    <div className="bg-white rounded-[2rem] p-12 shadow-soft border border-neutral-100 flex flex-col items-center justify-center text-center">
                        ...
                        <div className="flex gap-4 flex-wrap mt-8">
                            <button className="bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white px-8 py-3.5 rounded-full font-bold shadow-lg transition-transform hover:-translate-y-0.5" onClick={() => navigate('/reader')}>Abrir reader</button>
                            <button className="bg-white text-neutral-700 border border-neutral-200 hover:bg-neutral-50 px-8 py-3.5 rounded-full font-bold shadow-sm transition-transform hover:-translate-y-0.5" onClick={() => navigate('/practice')}>Abrir prática</button>
                        </div>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-8">
                        <div className="bg-violet-600 rounded-[2rem] p-8 text-white shadow-lg relative overflow-hidden flex flex-col">
                            ...
                            <div className="mt-auto">
                                ...
                                <button className="w-full bg-white text-violet-900 hover:bg-neutral-50 px-8 py-4 rounded-full font-extrabold shadow-lg transition-transform hover:-translate-y-1 flex items-center justify-center gap-2" onClick={startSession}>
                                    Iniciar revisão
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-3">
                                {dueCards.slice(0, 5).map((card) => (
                                    <div key={card.id} className="bg-white rounded-2xl p-5 border border-neutral-100 shadow-soft hover:border-violet-200 transition-colors flex flex-col gap-2 relative overflow-hidden">
                                        ...
                                        <div className="text-sm font-bold text-neutral-900 relative z-10">{card.front}</div>
                                        <div className="text-xs font-semibold text-neutral-500 relative z-10">{card.back}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
    ...
    return (
        <div className="text-neutral-800 antialiased min-h-screen flex flex-col pt-0 lg:pt-0 pb-16">
            <main className="w-full mt-2 lg:mt-4 text-center">
                ...
                <h2 className="text-4xl font-extrabold text-neutral-900 mb-4 tracking-tight">Sessão concluída</h2>
                ...
                <div className="bg-white rounded-[2rem] p-8 shadow-soft border border-neutral-100 mb-10 text-left">
                    <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-6">Resultado da sessão</div>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            ...
                            <div key={label} className={`${bgClass} rounded-2xl p-6 text-center border border-white/50 shadow-inner-soft`}>
                                <div className={`text-3xl font-black mb-2 ${colorClass}`}>{count}</div>
                                <div className="text-[11px] font-bold text-neutral-600 uppercase tracking-widest">{label}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex justify-center gap-4 flex-wrap">
                    <button className="bg-neutral-900 text-white hover:bg-black px-8 py-3.5 rounded-full font-bold shadow-sm transition-transform hover:-translate-y-0.5" onClick={() => setSession(null)}>Voltar ao painel</button>
                    <button className="bg-white text-neutral-700 border border-neutral-200 hover:bg-neutral-50 px-8 py-3.5 rounded-full font-bold shadow-sm transition-transform hover:-translate-y-0.5" onClick={() => navigate('/reader')}>Ler</button>
                    <button className="bg-white text-neutral-700 border border-neutral-200 hover:bg-neutral-50 px-8 py-3.5 rounded-full font-bold shadow-sm transition-transform hover:-translate-y-0.5" onClick={() => navigate('/practice')}>Praticar</button>
                </div>
            </main>
        </div>
    );
            ...
            <main className="w-full mt-2 lg:mt-4">
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-3 text-sm font-bold">
                        ...
                        <span className="bg-violet-100 text-violet-700 px-2 py-0.5 rounded text-[10px] uppercase tracking-wider border border-violet-200">{progress}%</span>
                    </div>
                    <div className="h-2 w-full bg-neutral-100 rounded-full overflow-hidden border border-neutral-200/50">
                        <div className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-400 transition-all duration-500" style={{ width: `${progress}%` }}></div>
                    </div>
                </div>
                ...
                <div className="flex justify-center mt-12">
                    <button className="text-sm font-bold text-neutral-400 hover:text-neutral-600 underline underline-offset-4" onClick={() => setDone(true)}>Encerrar sessão antecipadamente</button>
                </div>
            </main>
        </div>
  );
